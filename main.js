...
ipcMain.on('refresh-avaliable-archlinux-wsl-list-start', (event, msg) => {
    event.reply('refresh-avaliable-archlinux-wsl-list-start-complete', msg);
});

ipcMain.on('refresh-avaliable-archlinux-wsl-list-stop', (event, msg) => {
    const oneDayAgo = new Date(new Date().getTime() - 24 * 60 * 60 * 1000);
    const oneDayAgoStr = formatDateToYYYYMMDD(oneDayAgo);
    const archlinuxWslUrl = `https://github.com/CNOCTAVE/archlinux-wsl-manager-github-action-data-service/releases/download/${oneDayAgoStr}/archlinux_wsl_list.json`;
    var data = '';
    https.get(archlinuxWslUrl, function (res) {
        if (res.statusCode > 300 && res.statusCode < 400 && res.headers.location) {
            https.get(res.headers.location, function (res) {
                res.on('data', function (chunk) {
                    ...
                    if (data === '') {
                        event.reply('refresh-avaliable-archlinux-wsl-list-stop-complete', JSON.stringify([]));
                    } else {
                        event.reply('refresh-avaliable-archlinux-wsl-list-stop-complete', data);
                    }
...
ipcMain.on('choose-wsl-file-path-start', (event, msg) => {
    const files = dialog.showOpenDialogSync({
        properties: ['openFile'],
        filters: [
            { name: 'TAR/WSL Files', extensions: ['tar', 'wsl'] },
        ],
    });
